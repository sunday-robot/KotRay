...
object MovingSpheres {
            ...
            for (b in -11..10) {
                ...
                when {
                    chooseMat < 0.8 -> {// ざらついたプラスチックのような素材
                        val material = Lambertian(Rgb(random() * random(), random() * random(), random() * random()))
                        val center1 = center + Vec3(0.0, 0.5 * random(), 0.0)
                        hitables.add(MovingSphere(center, 0.0, center1, 1.0, 0.2, material))
                    }
                    chooseMat < 0.95 -> {// 金属
                        val material = Metal(Rgb(0.5 * (1 + random()), 0.5 * (1 + random()), 0.5 * (1 + random())),
                                0.5 * random())
    ...
    fun main(args: Array<String>) {
        ...
        try {
            saveAsPPM(NX, NY, pixels, "MovingSpheres.ppm")
